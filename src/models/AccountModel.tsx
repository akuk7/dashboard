...
interface Props {
  onClose: () => void
  onCreate: (name: string, openingBalance: number) => void
}

const AccountModal: React.FC<Props> = ({ onClose, onCreate }) => {
  const [name, setName] = useState('')
  const [openingBalance, setOpeningBalance] = useState('0')

  const handleCreate = () => {
    if (name.trim()) {
      onCreate(name.trim(), Number(openingBalance) || 0)
    }
  }
      ...
      <div className="bg-[#121212] text-gray-100 rounded-xl w-full max-w-sm p-6 border border-[#303030] shadow-2xl shadow-black/50">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold flex items-center gap-2 text-white">
            <PlusCircle className="w-5 h-5 text-gray-400" /> New Account
          </h3>
        ...
        <label className="block mb-2 text-sm font-medium text-gray-300">Account Name</label>
        <input
          type="text"
          value={name}
        ...
        <label className="block mb-2 text-sm font-medium text-gray-300">Opening Balance</label>
        <input
          type="number"
          step="0.01"
